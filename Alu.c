...
{
	char buff[BUFF_SIZE];
	unsigned char position1[4]={0};
	unsigned char position2[4]={0};
	unsigned char operation[2]={0};
	int pos1;
	int pos2;
	int value;
	int ret;
	...
	ret = sscanf(buff,"%[^,=]=%d",position1,&value);
	if(ret!=2)
	{
		ret=sscanf(buff,"%4s%*c%c%*c%4s",position1,operation,position2);
	}
	...
	{	
		...
		if(value >=0 && value <256)//two parameters parsed in sscanf
		{
			if(pos1 >=0 && pos1 <=3)
			{
				alu[pos1] = value; 
	...
	else if (ret==3)
	{
		...
		if(pos1 >=0 && pos1 <=3 && pos2>=0 && pos2<=3)
		{
			...
			{
				result=result-256;
				alu[4]=1; //alu[4] je pozicija overflow-a (carry)
				alu[5]=result; //alu[5] je pozicija rezultata
			}			
			else
			{
				alu[4]=0;
